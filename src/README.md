...
So that's the shape of every chapter: here's the failure, here's the minimal
...
coding agent in about 300 lines — and the durable knowledge that there's no
magic anywhere in the stack.

Production harnesses (Claude Code, and One Code, my reimplementation of it on
another agent runtime) show up as evidence that the patterns are real, never as
